 ...
 *
 **/
class TopRatedPagingSource (private val service: NetworkService): PagingSource<Int, Movie>(){
    ...
    override suspend fun load(params: PagingSource.LoadParams<Int>): PagingSource.LoadResult<Int, Movie> {
        ...
        return try {
            val data = NetworkRequest.process { service.getTopRatedMovies(key) }.run {
                when (this) {
            ...
            PagingSource.LoadResult.Page(movies, null, nextPage)
        } catch (e: Exception) {
