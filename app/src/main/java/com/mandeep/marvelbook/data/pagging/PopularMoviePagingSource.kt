 ...
 *
 **/
class PopularMoviePagingSource(private val service: NetworkService) : PagingSource<Int, Movie>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Movie> {
        ...
        return try {
            val data = NetworkRequest.process { service.getPopularMovies(key) }.run {
                when (this) {
                    is ApiResponse.Success -> {
                        results ?: throw Exception("Error while fetching popular Movies List")
                    }
